...
>
> **ข้อ 7 (เลื่อน full-res decode) และข้อ 8 (วัด DVFS) เขียนโค้ดเสร็จแล้วแต่ยังไม่มีข้อมูลจากเครื่องจริงแม้แต่รอบเดียว** — สิ่งที่ยืนยันได้ตอนนี้คือคอมไพล์ผ่านเท่านั้น · ยังค้าง: **TC-11 (live capture smoke test)** ซึ่งควรรันก่อนเพื่อน · **TC-05** (ตัวควบคุมที่ปิด halving) จึงยังแยกผลของข้อ 1 กับข้อ 4 ออกจากกันไม่ได้ · **TC-12/13** baseline ใหม่หลังข้อ 7
...
**ยังไม่ได้ทำ:** ปล่อย splitter วิ่งฟรีไม่ต้องรอคิว (split จะจบใน 17 s จริงๆ) — ต้องมีดิสก์ว่าง ~1.9 GB เทียบกับ ~420 MB ที่ cap 8 ใช้อยู่ ควรทำเป็นเงื่อนไขจาก `StatFs` ไม่ใช่เปิดตลอด
...
**ยังไม่ได้วัด** — `yuv_nv21` เทียบ `nv21_jpeg` เป็นตัวเลขที่ยังไม่เคยเห็น ยังไม่รู้ว่าที่เหลือบน producer ก้อนไหนใหญ่กว่า ถ้าเป็น `yuv_nv21` (loop per-pixel ใน Kotlin) ก็ยังมีทางไปต่อโดยไม่ต้องแตะ HardwareBuffer
...
| **TC-13** | `run4mins.mp4` | v0.1.4 · หลังข้อ 7/8 · `FRAME_QUEUE_CAPACITY = 2` | **แยกตัวแปรที่สองของข้อ 7 ออก** — เทียบกับ TC-12 บอกว่าคิวที่ลึกขึ้นให้อะไรจริงบ้าง |
...
| เรื่อง | ทำไมเว้นไว้ |
|--|--|
| **`HardwareBuffer` / `ImageFormat.PRIVATE`** *(NA-02 · OQ-02)* | **รอผลข้อ 7 ก่อน** — ARGB decode ซึ่งเคยเป็นก้อนใหญ่ที่สุดใน `yuv_jpeg_argb` ถูกย้ายออกจาก producer ไปแล้ว ที่เหลือคือ `yuv_nv21` + `nv21_jpeg` ซึ่งยังไม่เคยวัดแยก · ทางนี้เคยล้มเหลวมาแล้วครั้งหนึ่งและต้องยืนยันบนเครื่องจริง จึงไม่ควรแตะจนกว่าจะรู้ว่าเหลืออะไรให้แก้จริงๆ |
| **`yuv420ToNv21` — loop per-pixel ใน Kotlin** | ทางไปต่อที่ถูกที่สุด**ถ้า**ข้อ 7 ชี้ว่า `yuv_nv21` คือก้อนที่เหลือ · เป็น nested loop อ่าน `ByteBuffer.get(index)` ทีละ byte ตลอด chroma plane ซึ่งที่ UHD คือ ~2M ครั้งต่อเฟรม · เร่งได้ด้วย bulk `get()` ต่อ row เมื่อ `pixelStride == 1` โดยไม่ต้องแตะ NDK |
